...
* When you have completed the assignment, you need to  push your code to this repository and [mark the assignment as completed by clicking here](https://app.snapcode.review/submission_links/bc00253c-2253-4486-b5fc-46d1f6d39e9c).
...
The solution focuses on simplicity as requested in the assignment.

* **ShoppingCart** contains the core cart logic such as adding products and calculating subtotal, tax (12.5%), and total.
* **PriceAPIClient** retrieves product prices from the provided Price API.
* **Product** and **CartItem** represent the cart data model.

### Assumptions

* Tax rate is fixed at **12.5%**.
* All monetary values are rounded to two decimal places.
* Product prices are retrieved using the provided Price API.
* The product title from the API response is not used since the cart already tracks the product name.
* It is assumed that valid product names are provided and the API returns a valid price.
...
* Price API client behaviour

External API calls are mocked using **Mockito** to avoid real network calls.
...
* Assistance in identifying required dependencies
* Minor refactoring suggestions to improve code readability
* Assistance in writing and refining some unit tests

**Estimated Percentage of AI Generated Code**
Approximately **10–15%**

**Review Process**
All AI suggestions were manually reviewed, adjusted where needed, and verified by running the unit tests locally.
